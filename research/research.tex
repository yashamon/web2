...
\begin{theorem} \label{} Given $R>r$, there is an $\epsilon>0$ s.t. for any symplectic form $\omega' $ on $S ^{2} \times T ^{2n-2}  $ $C ^{0} $ $\epsilon$-close to a split symplectic form $\omega$, which satisfies: $$ \langle \omega, (A=[S ^{2} ] \otimes [pt])  \rangle = \pi r ^{2},  $$  there is no symplectic embedding $\phi: B _{R} \hookrightarrow (S ^{2} \times T ^{2n-2}, \omega')   $.
\end{theorem}
