...
Of course one exciting thing here is that it will be without so called ``hard'' elliptic pde techniques of pseudo-holomorphic curves. Instead the methods would just be conceptual methods of topology. It might be possible however that the proof of the conjecture above itself may initially involve elliptic techniques.
If we could verify this conjecture,  we would obtain new qualitative dynamical-topological information about Reeb vector fields.  
...
\begin{question} \label{quest:rect} Suppose $S$ that is rectifiable in the sense of measure theory, does the conclusion of the Conjecture \ref{conj:preliminary} hold in this case?
   \end{question}
Why does rectifiable help?  What we want to do 
If $S$ is locally rectifiable, then there are powerful additional tools at our disposal,  most important of which is Sullivan's theory \cite{cite:} which develops a connection between measure theory, and Hahn-Banach theorem with dynamics. Indeed under the rectifiable hypothesis it should be technically possible to just build on the proof of  Theorem \ref{}, to produce this special case of the conjecture.
