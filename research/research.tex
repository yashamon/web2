...
\begin{question} \label{quest:rect} Suppose $S$ that is rectifiable in the sense of measure theory, does the conclusion of the Conjecture \ref{conj:preliminary} hold in this case?
   \end{question}
If $S$ is rectifiable, then there are powerful additional tools at our disposal,  most important of which is Sullivan's theory \cite{cite:} which develops a connection between measure theory, and Hahn-Banach theorem with dynamics. Indeed under the rectifiable hypothesis it should be technically possible to just build on the proof of Theorem \ref{}, to produce this special case of the conjecture, but it should still be interesting and challenging.
