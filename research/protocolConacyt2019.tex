...
\begin{corollary} Assume the above conjecture.
   Let $f: S ^{1}  \to Cont (C)$ be a smooth family, for $C$ as above, with $Cont (C)$  the space of contact forms on $C$, i.e. $1$-forms $\lambda$ such that $\lambda \wedge \lambda ^{2k} \neq 0 $. Then there is an $S ^{1} $ family of Reeb orbits for $f$, meaning a continuous map $R: S ^{1} \to LC   $ s.t. $R (\theta)$ is a Reeb orbit of $f ^{n}  (\theta)$, for $LC$ the free loop space of $C$, $n \in \mathbb{N}$ and $f ^{} $
