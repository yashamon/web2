...
\section {An $\lcs$-homology theory} The above approach to existence of elliptic curves in $C \times S ^{1} $ via Seiberg-Witten theory is very partial, we need $C$ to be dimension 3, and we need topological conditions on $C$. For general $\lcs$ manifolds $M$ we need to develop an analogue of contact homology, denote by $CSH (M)$.
...
We propose that the analogue of a Reeb orbit, and hence of a generator of $\lcs$ homology of $M$, is a $A _{\omega} $-flat, $J _{\omega} $-holomorphic elliptic curve in $M$, we call them \textbf{\emph{calibrated elliptic curves}}. \footnote {Because flat has other connotations in algebraic geometry.
}
