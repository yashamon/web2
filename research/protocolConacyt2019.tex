...
\section {An $\lcs$-homology theory} The above approach to existence of elliptic curves in $C \times S ^{1} $ via Seiberg-Witten theory is very partial, we need $C$ to be dimension 3, and we need topological conditions on $C$. For general $\lcs$-manifolds we need to develop an analogue of contact homology. Indeed for the Banyaga $\lcs$ structure on $M=C\times S ^{1} $ with $(C$ contact, as in Example 1, for an appropriate almost complex structure all holomorphic tori, are in one to one correspondence with Reeb orbits in $C$
