...
\section {An $\lcs$-homology theory} The above approach to existence of elliptic curves in $C \times S ^{1} $ via Seiberg-Witten theory is very partial, we need $C$ to be dimension 3, and we need topological restrictions on $C$, otherwise $$ For general $\lcs$ manifolds $M$ we need to develop an analogue of contact homology, denoted by $CSH (M)$ for example. This is possibly work in collaboration with Yong-Geun Oh, of IBS, Pohang, Korea, as we held a number of discussions on this problem.
