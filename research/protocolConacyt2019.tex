...
\subsection {Goal 3} Prove holomorphic Weinstein conjecture for $M = C \times S ^{1} $ with $C$ a 3-fold, following the above outline.  \section {An $\lcs$-homology theory} The above approach to existence of elliptic curves in $C \times S ^{1} $ via Seiberg-Witten theory is very partial, we need $C$ to be dimension 3, and we need topological conditions on $C$. For general $\lcs$ manifolds $M$ we need to develop an analogue of contact homology, denote by $CSH (M)$.
...
Instantons (without compactifications) in this homology theory $CSH (M)$ are $J _{\omega} $-holomorphic cylinders in $M$, which are asymptotic in appropriate sense to calibrated elliptic curves, which is equivalent to a condition that they have finite energy, for the latter appropriately defined.
