...
Instantons (without compactifications) in this homology theory $CSH (M)$ are $J _{\omega} $-holomorphic cylinders in $M$, which are asymptotic in appropriate sense to calibrated elliptic curves. When $M$ is the Banyaga $\lcs$ structure we naturally have $$CSH (M) = CH (M).
 $$
\subsection {Goal 4}  Develop the homology theory $CSH (M)$, and use this to do prove $\lcs$ non-squeezing, as in Goal 2.a.  \bibliographystyle{siam}  \bibliography{/home/yasha/texmf/bibtex/bib/link} % \bibliography{/root/texmf/bibtex/bib/link} 
\end{document}
