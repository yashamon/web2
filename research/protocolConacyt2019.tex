...
For general $\lcs$ manifolds $M$ we need to develop an analogue of contact homology, denoted by $CSH (M)$ for example. This is possibly work in collaboration with Yong-Geun Oh, of IBS, Pohang, Korea, as we held a number of discussions on this problem.  
 ...
 Indeed for the Banyaga $\lcs$ structure on $M=C\times S ^{1} $ with $(C, \lambda)$ contact, as in Example 1, for an appropriate almost complex structure $J _{\lambda} $ all $J _{\lambda} $-holomorphic tori, are in one to one correspondence with Reeb orbits of $(C, \lambda)$. They are just products of Reeb orbits by the $S ^{1} $ factor of $M \times S ^{1} $.  But these Reeb tori as we call them have an additional structure.   There is natural symplectic connection $A _{\lambda} $ on  $T(M \times S ^{1}) $, determined by $\lambda$, such that all Reeb tori are flat for $A _{\lambda} $. 

 For a more general $\lcs$ structure $\omega$ on $M \times S ^{1} $, if there is a fibration $M \times S ^{1} \to S ^{1} $ with all fibers contact type,  we can an analogous symplectic connection $A _{\omega} $, with an appropriately compatible almost complex structure $J _{\omega} $.
