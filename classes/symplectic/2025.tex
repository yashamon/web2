...
\begin{env}
Show that with our definition each fiber of a vector bundle
has a natural (meaning uniquely determined) structure of
a vector space.	
\end{env}
\begin{problem}
Show that the space of sections of a vector bundle is
naturally a vector space.
Show that there is an isomorphism between the vector space
of smooth functions on a manifold $X$, and the set of
smooth sections of the trivial bundle $X \times \mathbb{R} ^{} $ over $X$.
\end{problem}
\begin{problem}
Let $M$ be the open Mobious band (remove the boundary
circle). There is a natural projection $M \to  S ^{1}$ is
this a (topological) vector bundle? Topological means that
the fibered bundle charts are only required to be continuous. 
\end{problem}
